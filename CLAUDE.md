...
**Work hours are 24-hour `HH:mm` strings, invariant-culture.** `Calendar.StartTime`/`EndTime` are stored as strings, so the format is pinned at both ends rather than left to the ambient locale: `EditCalendarRequest` enforces `^([01]\d|2[0-3]):[0-5]\d$` with a `[RegularExpression]` annotation (rejected by `[ApiController]`'s automatic 400 before it reaches the service), and every `TimeOnly.TryParse` of those columns passes `CultureInfo.InvariantCulture` — `AppointmentService.ValidateBookingRules` on the server, `Booking.IsAppointmentWithinWorkHours` on the client. `EditCalendarViewModel.ToRequest` formats with `"HH:mm"` + `InvariantCulture` for the same reason, and `EditCalendarViewModel.Create` parses back with `TimeOnly.ParseExact`. Both `Booker.Services` and `Booker.Clients.Blazor.Server` import `CultureInfo` as a **global alias**, not `global using System.Globalization` — that namespace has its own `Calendar` type which collides with `Booker.Repository.Entities.Calendar`.

**Editing a calendar's work hours is blocked while bookings would be stranded.** `CalendarService.UpdateCalendar` rejects the edit when any *upcoming* appointment on the calendar starts before the new `StartTime` or ends after the new `EndTime`, returning a count in the error message rather than silently leaving appointments outside the window. The filter uses `DateTime.Now` (see the wall-clock note above), so past appointments never block an edit. `CalendarService` also enforces `StartTime < EndTime` on both `AddCalendar` and `UpdateCalendar` via the shared `ParseWorkHours` helper — the `[RegularExpression]` annotation only validates the *format* of each field, not their order. The Blazor pages pre-check the same order rule with `EditCalendarViewModel.IsWorkHoursOrderValid` purely to avoid a round-trip; the server remains the authority.
